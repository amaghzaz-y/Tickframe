...
module internal RegistryHelpers =
    ...
    let toReusable (frame: OhlcvFrame) (values: float[]) : System.Collections.Generic.IReadOnlyList<IReusable> =
        let bars =
            [| for i in 0 .. values.Length - 1 ->
                let v = values.[i]
                let d = if Double.IsNaN v || Double.IsInfinity v then 0m else decimal v
    ...
    let toBars (frame: OhlcvFrame) (mapped: decimal[][]) (slotCount: int) : System.Collections.Generic.IReadOnlyList<IBar> =
        [| for i in 0 .. frame.RowCount - 1 ->
            let ts = frame.Candles.[i].Timestamp
            let get (idx: int) = if idx < mapped.Length then mapped.[idx].[i] else 0m
    ...
    let toBarsOHLCV (frame: OhlcvFrame) (opens: decimal[]) (highs: decimal[]) (lows: decimal[]) (closes: decimal[]) (volumes: decimal[]) =
        [| for i in 0 .. frame.RowCount - 1 ->
    ...
    let resolveOperands (ctx: EvalContext) (call: IndicatorCall) (slots: InputSlots) : float[][] =
        ...
        let refs =
            if call.Series.IsEmpty then defaults |> List.map SeriesColumn
            else call.Series
        let expected = defaults.Length
        if refs.Length <> expected then
            raise (DirectiveValueError $"indicator '{call.Name}' expects {expected} series operand(s), got {refs.Length}")
        refs |> List.map (fun r -> resolveFloatSeries ctx r) |> List.toArray

    let floatsToDecimals (values: float[]) : decimal[] =
        values |> Array.map (fun v -> if Double.IsNaN v || Double.IsInfinity v then 0m else decimal v)

    let toNullableFloat (v: Nullable<double>) : float =
        if v.HasValue then v.Value else Double.NaN

    let toFloat (v: double) : float = v
    let toFloatN (v: Nullable<double>) : float = if v.HasValue then v.Value else Double.NaN
    let toFloatDecimal (v: Nullable<decimal>) : float = if v.HasValue then float v.Value else Double.NaN
    ...
    let parseDecimal (call: IndicatorCall) (idx: int) (defaultValue: decimal) : decimal =
        ...
        if s = "" then defaultValue
            ...
            match Decimal.TryParse(s, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture) with
            | true, v -> v
            | _ -> raise (DirectiveValueError $"argument '{s}' is not a number")

    let requireArg (call: IndicatorCall) (idx: int) (name: string) : string =
        let s = arg call idx
        if s = "" then raise (DirectiveValueError $"indicator '{call.Name}' requires argument {idx + 1} ({name}); empty slots are not allowed")
        s
    ...
    let nullableBool (v: Nullable<bool>) : bool =
        if v.HasValue then v.Value else false

module Registry =

    open RegistryHelpers

    let private mk (name: string) (aliases: string list) (slots: InputSlots) (outputKind: IndicatorOutputKind)
                  (compute: IndicatorCall -> EvalContext -> Series) (lookback: IndicatorCall -> int)
                  (subs: Map<string, IndicatorSpec>) : IndicatorSpec =
        { Name = name; Aliases = aliases; SubCommands = subs; Slots = slots; OutputKind = outputKind; Compute = compute; Lookback = lookback }

    let private leaf (name: string) (slots: InputSlots) (compute: IndicatorCall -> EvalContext -> Series) (lookback: IndicatorCall -> int) : IndicatorSpec =
        { Name = name; Aliases = []; SubCommands = Map.empty; Slots = slots; OutputKind = FloatOutput; Compute = compute; Lookback = lookback }

    let buildTable () : Map<string, IndicatorSpec> =

        let smaLike (name: string) (aliases: string list) (toFn: System.Collections.Generic.IReadOnlyList<IReusable> * int -> System.Collections.Generic.IReadOnlyList<'R>) (getValue: 'R -> float) (defaultPeriod: int) : IndicatorSpec =
            mk name aliases CloseOnly FloatOutput
                (fun call ctx ->
        ...
        let smaSpec = smaLike "ma" ["sma"] (fun (src, p) -> Sma.ToSma(src, p)) (fun (r: SmaResult) -> toFloatN r.Sma) 20
        let emaSpec = smaLike "ema" [] (fun (src, p) -> Ema.ToEma(src, p)) (fun (r: EmaResult) -> toFloatN r.Ema) 20
        let wmaSpec = smaLike "wma" [] (fun (src, p) -> Wma.ToWma(src, p)) (fun (r: WmaResult) -> toFloatN r.Wma) 20
        let hmaSpec = smaLike "hma" [] (fun (src, p) -> Hma.ToHma(src, p)) (fun (r: HmaResult) -> toFloatN r.Hma) 14
        let demaSpec = smaLike "dema" [] (fun (src, p) -> Dema.ToDema(src, p)) (fun (r: DemaResult) -> toFloatN r.Dema) 20
        let temaSpec = smaLike "tema" [] (fun (src, p) -> Tema.ToTema(src, p)) (fun (r: TemaResult) -> toFloatN r.Tema) 20
        let smmaSpec = smaLike "smma" ["mma"; "rma"] (fun (src, p) -> Smma.ToSmma(src, p)) (fun (r: SmmaResult) -> toFloatN r.Smma) 20
        let epmaSpec = smaLike "epma" ["lsma"] (fun (src, p) -> Epma.ToEpma(src, p)) (fun (r: EpmaResult) -> toFloatN r.Value) 10

        let almaSpec =
            mk "alma" [] CloseOnly FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 9) Map.empty

        let kamaSpec =
            mk "kama" [] CloseOnly FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 10) Map.empty
        ...
        let mamaSpec =
            mk "mama" [] CloseOnly FloatOutput
                ...
                (fun _ -> 0)
                (Map.ofList ["fama", { Name = "fama"; Aliases = []; SubCommands = Map.empty; Slots = CloseOnly; OutputKind = FloatOutput; Compute = mamaFamaCompute; Lookback = fun _ -> 0 }])

        let t3Spec =
            mk "t3" [] CloseOnly FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 5) Map.empty

        let mgDynSpec =
            mk "dynamic" [] CloseOnly FloatOutput
                (fun call ctx ->
                    let period = parseInt call 0 20
                    let k = parseFloat call 1 0.6
                    let s = arg call 0
                    if s = "" then raise (DirectiveValueError "indicator 'dynamic' requires argument 1 (period)")
                    let ops = resolveOperands ctx call CloseOnly
                    let res = MgDynamic.ToDynamic(toReusable ctx.Frame ops.[0], period, k)
                    projectFloat res (fun (r: DynamicResult) -> toFloatN r.Value))
                (fun call -> let _ = requireArg call 0 "period" in parseInt call 0 20) Map.empty

        let rsiSpec = smaLike "rsi" [] (fun (src, p) -> Rsi.ToRsi(src, p)) (fun (r: RsiResult) -> toFloatN r.Rsi) 14
        let cmoSpec = smaLike "cmo" [] (fun (src, p) -> Cmo.ToCmo(src, p)) (fun (r: CmoResult) -> toFloatN r.Cmo) 14
        let trixSpec = smaLike "trix" [] (fun (src, p) -> Trix.ToTrix(src, p)) (fun (r: TrixResult) -> toFloatN r.Trix) 14
        let rocSpec = smaLike "roc" [] (fun (src, p) -> Roc.ToRoc(src, p)) (fun (r: RocResult) -> toFloatN r.Roc) 14

        let rocWbSpec =
            mk "roc-wb" ["rocwb"] CloseOnly FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 20) Map.empty

        let stcSpec =
            mk "stc" [] CloseOnly FloatOutput
                (fun call ctx ->
        ...
        let connorsSpec =
            mk "connors-rsi" ["connorsrsi"] CloseOnly FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 2 100) Map.empty

        let stochRsiSpec =
            mk "stoch-rsi" ["stochrsi"] CloseOnly FloatOutput
                (fun call ctx ->
                    ...
                    let ops = resolveOperands ctx call CloseOnly
                    let res = StochRsi.ToStochRsi(toReusable ctx.Frame ops.[0], rsi, stoch, sig_, smooth)
                    match call.Sub with
                ...
                (fun _ -> 14) (Map.ofList ["k", leaf "k" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0); "d", leaf "d" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0)])

        let hurstSpec = smaLike "hurst" [] (fun (src, p) -> Hurst.ToHurst(src, p)) (fun (r: HurstResult) -> toFloatN r.HurstExponent) 100
        let ulcerSpec = smaLike "ulcer-index" ["ulcerindex"] (fun (src, p) -> UlcerIndex.ToUlcerIndex(src, p)) (fun (r: UlcerIndexResult) -> toFloatN r.UlcerIndex) 14
        let fisherSpec = smaLike "fisher-transform" ["fishertransform"] (fun (src, p) -> FisherTransform.ToFisherTransform(src, p)) (fun (r: FisherTransformResult) -> r.Fisher) 10

        let stdDevSpec =
            mk "std-dev" ["stddev"] CloseOnly FloatOutput
                (fun call ctx ->
                ...
                (Map.ofList ["zscore", leaf "zscore" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0); "mean", leaf "mean" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0)])

        let slopeSpec = smaLike "slope" [] (fun (src, p) -> Slope.ToSlope(src, p)) (fun (r: SlopeResult) -> toFloatN r.Slope) 14
        let htSpec =
            mk "ht-trendline" ["httrendline"] CloseOnly FloatOutput
                (fun call ctx ->
                    let ops = resolveOperands ctx call CloseOnly
        ...
        let dpoSpec = smaLike "dpo" [] (fun (src, p) -> Dpo.ToDpo(src, p)) (fun (r: DpoResult) -> toFloatN r.Dpo) 20

        let barCloseOnly (toFn: System.Collections.Generic.IReadOnlyList<IBar> * int -> System.Collections.Generic.IReadOnlyList<'R>) (getValue: 'R -> float) (defaultPeriod: int) : (IndicatorCall -> EvalContext -> Series) * (IndicatorCall -> int) =
        ...
        let atrSpec =
            let c, l = barCloseOnly (fun (b, p) -> Atr.ToAtr(b, p)) (fun (r: AtrResult) -> toFloatN r.Atr) 14
            mk "atr" [] HighLowClose FloatOutput c l Map.empty

        let natrSpec =
            mk "natr" [] HighLowClose FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 14
                    let ops = resolveOperands ctx call HighLowClose
                    let decs = ops |> Array.map floatsToDecimals
                ...
                (fun call -> parseInt call 0 14) Map.empty

        let trSpec =
            mk "tr" [] HighLowClose FloatOutput
                (fun call ctx ->
                    let ops = resolveOperands ctx call HighLowClose
                    let decs = ops |> Array.map floatsToDecimals
                    let bars = toBars ctx.Frame decs 3
        ...
        let adxSpec =
            mk "adx" ["dmi"] HighLowClose FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 14
                    let ops = resolveOperands ctx call HighLowClose
                    let decs = ops |> Array.map floatsToDecimals
        ...
        let aroonSpec =
            mk "aroon" [] HighLow FloatOutput
                ...
                (fun call -> parseInt call 0 25)
                (Map.ofList ["up", leaf "up" HighLow (fun _ _ -> failwith "") (fun _ -> 0); "down", leaf "down" HighLow (fun _ _ -> failwith "") (fun _ -> 0)])

        let cciSpec =
            let c, l = barCloseOnly (fun (b, p) -> Cci.ToCci(b, p)) (fun (r: CciResult) -> toFloatN r.Cci) 20
            mk "cci" [] HighLowClose FloatOutput c l Map.empty

        let chopSpec =
            let c, l = barCloseOnly (fun (b, p) -> Chop.ToChop(b, p)) (fun (r: ChopResult) -> toFloatN r.Chop) 14
            mk "chop" [] HighLowClose FloatOutput c l Map.empty

        let bopSpec =
            mk "bop" [] OpenHighLowClose FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 14
        ...
        let ultimateSpec =
            mk "ultimate" [] HighLowClose FloatOutput
                (fun call ctx ->
        ...
        let williamsSpec =
            let c, l = barCloseOnly (fun (b, p) -> WilliamsR.ToWilliamsR(b, p)) (fun (r: WilliamsResult) -> toFloatN r.WilliamsR) 14
            mk "williams-r" ["williamsr"] HighLowClose FloatOutput c l Map.empty

        let stochSpec =
            mk "stoch" [] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 14)
                (Map.ofList ["k", leaf "k" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "d", leaf "d" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "j", leaf "j" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let kdjSpec =
            mk "kdj" [] HighLowClose FloatOutput
                (fun call ctx ->
                    let lb = parseInt call 0 14
                    let sig_ = parseInt call 1 3
                    let sm = parseInt call 2 3
        ...
        let smiSpec =
            mk "smi" [] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun _ -> 13+25) (Map.ofList ["signal", leaf "signal" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let bollSpec =
            mk "boll" [] CloseOnly FloatOutput
                (fun call ctx ->
                    let lb = parseInt call 0 20
                ...
                (Map.ofList ["upper", leaf "upper" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0); "lower", leaf "lower" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0); "middle", leaf "middle" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0)])

        let donchianSpec =
            mk "donchian" [] HighLow FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 20)
                (Map.ofList ["upper", leaf "upper" HighLow (fun _ _ -> failwith "") (fun _ -> 0); "lower", leaf "lower" HighLow (fun _ _ -> failwith "") (fun _ -> 0); "middle", leaf "middle" HighLow (fun _ _ -> failwith "") (fun _ -> 0)])

        let keltnerSpec =
            mk "keltner" [] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 20)
                (Map.ofList ["upper", leaf "upper" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "lower", leaf "lower" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "middle", leaf "middle" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let starcSpec =
            mk "starc-bands" ["starcbands"] HighLowClose FloatOutput
                (fun call ctx ->
        ...
        let fcbSpec =
            mk "fcb" [] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 2) (Map.ofList ["upper", leaf "upper" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "lower", leaf "lower" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let maEnvSpec =
            mk "ma-envelopes" ["maenvelopes"] CloseOnly FloatOutput
                (fun call ctx ->
                    let lb = parseInt call 0 20
                ...
                (fun call -> parseInt call 0 20) (Map.ofList ["upper", leaf "upper" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0); "lower", leaf "lower" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0)])

        let stdDevChSpec =
            mk "std-dev-channels" ["stddevchannels"] CloseOnly FloatOutput
                (fun call ctx ->
                    let lb = parseInt call 0 20
                    let sd = parseFloat call 1 2.0
                    let ops = resolveOperands ctx call CloseOnly
                ...
                (fun call -> parseInt call 0 20) (Map.ofList ["upper", leaf "upper" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0); "lower", leaf "lower" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0); "middle", leaf "middle" CloseOnly (fun _ _ -> failwith "") (fun _ -> 0)])

        let ichimokuSpec =
            mk "ichimoku" [] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun _ -> 52) (Map.ofList ["tenkan", leaf "tenkan" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "kijun", leaf "kijun" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "senkou-a", leaf "senkou-a" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "senkou-b", leaf "senkou-b" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "chikou", leaf "chikou" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let superTrendSpec =
            mk "super-trend" ["supertrend"] HighLowClose FloatOutput
                (fun call ctx ->
                    ...
                    match call.Sub with
                    | Some "direction" -> projectBool res (fun (r: SuperTrendResult) -> r.UpperBand.HasValue && r.LowerBand.HasValue)
                    | _ -> projectFloat res (fun (r: SuperTrendResult) -> toFloatDecimal r.SuperTrend))
                (fun call -> parseInt call 0 10) (Map.ofList ["direction", { leaf "direction" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0) with OutputKind = BoolOutput }])

        let vortexSpec =
            mk "vortex" [] HighLowClose FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 14
                    let ops = resolveOperands ctx call HighLowClose
                    let decs = ops |> Array.map floatsToDecimals
                ...
                (fun call -> parseInt call 0 14) (Map.ofList ["plus", leaf "plus" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "minus", leaf "minus" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let alligatorSpec =
            mk "alligator" [] CloseOnly FloatOutput
                (fun call ctx ->
                    ...
                    let ops = resolveOperands ctx call CloseOnly
                    let res = Alligator.ToAlligator(toReusable ctx.Frame ops.[0], jaw, jawOff, teeth, teethOff, lips, lipsOff)
                    projectFloat res (fun (r: AlligatorResult) -> toFloatN r.Jaw))
                (fun _ -> 13+8) Map.empty

        let elderRaySpec =
            mk "elder-ray" ["elderray"] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 13) (Map.ofList ["bull", leaf "bull" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "bear", leaf "bear" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let gatorSpec =
            mk "gator" [] CloseOnly FloatOutput
                (fun call ctx ->
                    let ops = resolveOperands ctx call CloseOnly
        ...
        let awesomeSpec =
            mk "awesome" [] CloseOnly FloatOutput
                (fun call ctx ->
        ...
        let atrStopSpec =
            mk "atr-stop" ["atrstop"] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 21) Map.empty

        let chandelierSpec =
            mk "chandelier" [] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 22) Map.empty

        let parabolicSpec =
            mk "parabolic-sar" ["parabolicsar"] HighLow FloatOutput
                (fun call ctx ->
        ...
        let volatilityStopSpec =
            mk "volatility-stop" ["volatilitystop"] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 7) Map.empty

        let dojiSpec =
            mk "doji" [] OpenHighLowClose BoolOutput
                (fun call ctx ->
        ...
        let marubozuSpec =
            mk "marubozu" [] OpenHighLowClose BoolOutput
                (fun call ctx ->
        ...
        let pivotsSpec =
            mk "pivots" [] HighLowClose BoolOutput
                (fun call ctx ->
        ...
        let adlSpec =
            mk "adl" [] HighLowCloseVolume FloatOutput
                (fun call ctx ->
        ...
        let cmfSpec =
            mk "cmf" [] HighLowCloseVolume FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 20
                    let ops = resolveOperands ctx call HighLowCloseVolume
                    let decs = ops |> Array.map floatsToDecimals
        ...
        let chaikinSpec =
            mk "chaikin-osc" ["chaikinosc"] HighLowCloseVolume FloatOutput
                (fun call ctx ->
        ...
        let forceIndexSpec =
            mk "force-index" ["forceindex"] CloseVolume FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 2
                    let ops = resolveOperands ctx call CloseVolume
                    let closes = ops.[0] |> floatsToDecimals
                    let vols = ops.[1] |> floatsToDecimals
                    let bars = toBarsOHLCV ctx.Frame (Array.create ctx.Frame.RowCount 0m) (Array.create ctx.Frame.RowCount 0m) (Array.create ctx.Frame.RowCount 0m) closes vols
                    let res = ForceIndex.ToForceIndex(bars, p)
                    projectFloat res (fun (r: ForceIndexResult) -> toFloatN r.ForceIndex))
                (fun call -> parseInt call 0 2) Map.empty

        let kvoSpec =
            mk "kvo" [] HighLowCloseVolume FloatOutput
                (fun call ctx ->
                ...
                (fun _ -> 55) (Map.ofList ["signal", leaf "signal" HighLowCloseVolume (fun _ _ -> failwith "") (fun _ -> 0)])

        let mfiSpec =
            mk "mfi" [] HighLowCloseVolume FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 14
                    let ops = resolveOperands ctx call HighLowCloseVolume
                    let decs = ops |> Array.map floatsToDecimals
        ...
        let obvSpec =
            mk "obv" [] CloseVolume FloatOutput
                (fun call ctx ->
                    let ops = resolveOperands ctx call CloseVolume
                    let closes = ops.[0] |> floatsToDecimals
                    let vols = ops.[1] |> floatsToDecimals
        ...
        let pvoSpec =
            mk "pvo" [] CloseVolume FloatOutput
                (fun call ctx ->
                    let fast = parseInt call 0 12
                    let slow = parseInt call 1 26
                    let sig_ = parseInt call 2 9
                ...
                (fun _ -> 26) (Map.ofList ["signal", leaf "signal" CloseVolume (fun _ _ -> failwith "") (fun _ -> 0)])

        let vwapSpec =
            mk "vwap" [] HighLowCloseVolume FloatOutput
                (fun call ctx ->
                    let ops = resolveOperands ctx call HighLowCloseVolume
                    let decs = ops |> Array.map floatsToDecimals
                    let bars = toBars ctx.Frame decs 4
        ...
        let vwmaSpec =
            mk "vwma" [] CloseVolume FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 20
                    let ops = resolveOperands ctx call CloseVolume
                    let closes = ops.[0] |> floatsToDecimals
        ...
        let barPartSpec =
            mk "bar-part" ["barpart"] HighLowClose FloatOutput
                (fun call ctx ->
                    let part =
        ...
        let heikinSpec =
            mk "heikin-ashi" ["heikinashi"] OpenHighLowClose FloatOutput
                (fun call ctx ->
                    let ops = resolveOperands ctx call OpenHighLowClose
                    let decs = ops |> Array.map floatsToDecimals
                    let bars = toBars ctx.Frame decs 4
                ...
                (fun _ -> 0) (Map.ofList ["open", leaf "open" OpenHighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "high", leaf "high" OpenHighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "low", leaf "low" OpenHighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "close", leaf "close" OpenHighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let renkoSpec =
            mk "renko" [] HighLowCloseVolume FloatOutput
                (fun call ctx ->
                    let size = parseDecimal call 0 1m
                    let s = arg call 0
                    if s = "" then raise (DirectiveValueError "indicator 'renko' requires argument 1 (brick size)")
                    let ops = resolveOperands ctx call HighLowCloseVolume
                    let decs = ops |> Array.map floatsToDecimals
                    let bars = toBars ctx.Frame decs 4
        ...
        let zigzagSpec =
            mk "zig-zag" ["zigzag"] HighLowClose FloatOutput
                (fun call ctx ->
        ...
        let betaSpec =
            mk "beta" [] CloseOnly FloatOutput
                (fun call ctx ->
                    ...
                    let src = toReusable ctx.Frame ops.[0]
                    let mrkt =
                        if call.Series.Length = 2 then toReusable ctx.Frame (resolveFloatSeries ctx call.Series.[1])
                        else src
        ...
        let corrSpec =
            mk "correlation" [] CloseOnly FloatOutput
                (fun call ctx ->
                    let p = parseInt call 0 20
                    let ops =
                        if call.Series.Length = 2 then
                            [| resolveFloatSeries ctx call.Series.[0]; resolveFloatSeries ctx call.Series.[1] |]
                        else
        ...
        let prsSpec =
            mk "prs" [] CloseOnly FloatOutput
                (fun call ctx ->
                    ...
                    let b = toReusable ctx.Frame ops.[1]
                    let s = arg call 0
                    let res = if s = "" then Prs.ToPrs(a, b) else Prs.ToPrs(a, b, parseInt call 0 14)
                    projectFloat res (fun (r: PrsResult) -> toFloatN r.Prs))
                (fun call -> let s = arg call 0 in if s = "" then 0 else parseInt call 0 14) Map.empty

        let pivotPointsSpec =
            mk "pivot-points" ["pivotpoints"] HighLowClose FloatOutput
                (fun call ctx ->
                    let ops = resolveOperands ctx call HighLowClose
                    let decs = ops |> Array.map floatsToDecimals
                    let bars = toBars ctx.Frame decs 3
                ...
                (fun _ -> 0) (Map.ofList ["pp", leaf "pp" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "r1", leaf "r1" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0); "s1", leaf "s1" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let rollingPivotsSpec =
            mk "rolling-pivots" ["rollingpivots"] HighLowClose FloatOutput
                (fun call ctx ->
                ...
                (fun call -> parseInt call 0 20) (Map.ofList ["pp", leaf "pp" HighLowClose (fun _ _ -> failwith "") (fun _ -> 0)])

        let increaseCompute (call: IndicatorCall) (ctx: EvalContext) =
            let n = parseInt call 0 3
            let s = arg call 0
            if s = "" then raise (DirectiveValueError "indicator 'increase' requires argument 1 (periods)")
            let ops = resolveOperands ctx call CloseOnly
            let a = ops.[0]
            ...
            for i in 0 .. a.Length - 1 do
                if i >= n then
                    let mutable ok = true
                    for k in 1 .. n do
                        if not (a.[i - k + 1] > a.[i - k]) then ok <- false
                    out.[i] <- ok
            Bool out
        let increaseSpec =
        ...
        let repeatCompute (call: IndicatorCall) (ctx: EvalContext) =
            let n = parseInt call 0 3
            let s = arg call 0
            if s = "" then raise (DirectiveValueError "indicator 'repeat' requires argument 1 (periods)")
            if call.Series.IsEmpty then raise (DirectiveValueError "indicator 'repeat' requires a bool series operand")
            let b =
        ...
        let all = [
            smaSpec; emaSpec; wmaSpec; hmaSpec; demaSpec; temaSpec; smmaSpec; epmaSpec; almaSpec; kamaSpec; mamaSpec; t3Spec; mgDynSpec
            rsiSpec; cmoSpec; trixSpec; rocSpec; rocWbSpec; stcSpec; pmoSpec; macdSpec; tsiSpec; connorsSpec; stochRsiSpec; hurstSpec; ulcerSpec; fisherSpec; stdDevSpec; slopeSpec; htSpec; dpoSpec
            atrSpec; natrSpec; trSpec; adxSpec; aroonSpec; cciSpec; chopSpec; bopSpec; ultimateSpec; williamsSpec; stochSpec; kdjSpec; smiSpec
            bollSpec; donchianSpec; keltnerSpec; starcSpec; fcbSpec; maEnvSpec; stdDevChSpec
            ichimokuSpec; superTrendSpec; vortexSpec; alligatorSpec; elderRaySpec; gatorSpec; awesomeSpec
            atrStopSpec; chandelierSpec; parabolicSpec; volatilityStopSpec
            dojiSpec; marubozuSpec; pivotsSpec; fractalSpec
            adlSpec; cmfSpec; chaikinSpec; forceIndexSpec; kvoSpec; mfiSpec; obvSpec; pvoSpec; vwapSpec; vwmaSpec
            barPartSpec; heikinSpec; renkoSpec; zigzagSpec
            betaSpec; corrSpec; prsSpec; pivotPointsSpec; rollingPivotsSpec
            increaseSpec; repeatSpec
        ]
        all
